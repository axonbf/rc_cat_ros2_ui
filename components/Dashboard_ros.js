...
import SimpleMap from "./SimpleMapVisualization"
import { getBatteryPercentage } from "./functions/BatteryStatus";

export default function Dashboard_ros() {

    const { connected, pointCloudData, gpsPosition, batteryStatus } = RosComponent();

    //let progress = 50;
    //const voltage = 12.4;  // You can change this to any voltage value
    //console.log(getBatteryPercentage(batteryStatus?.voltage));  // Output: Battery percentage based on the voltage
    return (
        <div className=" h-5/6 mx-auto w-11/12 grid grid-cols-4 gap-4 grid-rows-2 p-7">
            ...
            <div className="col-start-2 col-end-4 row-start-1 row-end-3 bg-[#000] p-4 rounded shadow-md z-[3]">
                ...
                <SimpleMap gpsPosition={gpsPosition} />
            </div>
            ...
            <div className="bg-[#0b0b0c] bg-opacity-50 p-4 rounded shadow-md flex flex-row relative overflow-hidden height-auto text-foreground box-border backdrop-blur-md rounded-lg saturate-100 z-[3] ">
                <div className="basis-1/2 h-full w-full">
                    ...
                    <Vol_progressBar percentage={getBatteryPercentage(batteryStatus?.voltage)} />
                </div>
